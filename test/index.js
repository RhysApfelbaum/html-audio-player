...
class AudioMotionAnalyzer {
    ...
    set peakFadeTime(value) {
        this._peakFadeTime =
            value >= 0
                ? +value
                : this._peakFadeTime || DEFAULT_SETTINGS.peakFadeTime;
    }
    get peakHoldTime() {
    ...
    _createScales() {
        ...
        if (!_noteLabels && (this._ansiBands || _frequencyScale != SCALE_LOG)) {
            ...
            else freqLabels.push(8000, 16000);
        } else {
            let freq = C_1;
        ...
        const centerR = canvasR.width >> 1,
            radialY = centerR - scaleHeight * 0.7;
        ...
        for (const item of freqLabels) {
            ...
            if (_noteLabels) {
                ...
                if (
                    ...
                    (!isLinear && freq > 500) ||
                    ((!_radial || isDualVertical) &&
                        ((!isLinear && freq > 250) || freq > 2000))
                )
                    allowedLabels.push('E');
                if (
                    (isLinear && freq > 4000) ||
                    ((!_radial || isDualVertical) &&
                        (isLog || freq > 2000 || (!isLinear && freq > 500)))
                )
                    allowedLabels.push('D', 'F', 'A', 'B');
                if (!allowedLabels.includes(label[0])) continue;
            }
    ...
    _draw(timestamp) {
        this._runId = requestAnimationFrame((timestamp2) =>
            this._draw(timestamp2)
        );
        const elapsed = timestamp - this._time,
            frameTime = timestamp - this._last,
            targetInterval = this._maxFPS ? 975 / this._maxFPS : 0;
        ...
        const {
            ...
            fadeFrames = (_fps * this._peakFadeTime) / 1000,
            fpsSquared = _fps ** 2,
            gravity = this._gravity * 1000,
            holdFrames = (_fps * this._peakHoldTime) / 1000,
            ...
            switch (_weightingFilter) {
                ...
                case FILTER_D:
                    const h =
                        ...
                        rD =
                            (freq / 0.000068966888496476) *
                            Math.sqrt(h / ((f2 + 79919.29) * (f2 + 1345600)));
                    return linearTodB(rD);
                case FILTER_468:
                    const h1 =
                            -0.000000000000000000000004737338981378384 *
                                freq ** 6 +
                            0.000000000000002043828333606125 * freq ** 4 -
                            0.0000001363894795463638 * f2 +
                            1,
                        h2 =
                            0.0000000000000000001306612257412824 * freq ** 5 -
                            0.00000000002118150887518656 * freq ** 3 +
                            0.0005559488023498642 * freq,
                        rI =
                            (0.0001246332637532143 * freq) / Math.hypot(h1, h2);
        ...
        for (let channel = 0; channel < nChannels; channel++) {
            const { channelTop, channelBottom, analyzerBottom } =
                ...
                angularDirection = isDualHorizontal && invertedChannel ? -1 : 1;
            const drawScaleY = () => {
                ...
                if (bar.peak[channel] > 0 && bar.alpha[channel] > 0) {
                    ...
                    if (bar.hold[channel] < 0) {
                        if (_fadePeaks && !showPeakLine) {
                            ...
                            bar.alpha[channel] =
                                initialAlpha *
                                (1 + bar.hold[channel] / fadeFrames);
                        } else
                            bar.peak[channel] +=
                                (bar.hold[channel] * gravity) /
                                fpsSquared /
                                Math.abs(nominalMaxHeight);
...
The selector '${options.selector}' returned no results.`);
        ...
        this.visualizer = new audioMotion_analyzer_default(
